"""
Microsoft Services Admin - DSP (Digital Solutions Platform)

Dieses Modul enthält die Django Admin-Konfiguration für Microsoft OAuth States.
Ermöglicht Monitoring, Management und Cleanup von OAuth States über das Admin-Interface.

Features:
- Übersichtliche Listendarstellung und Filteroptionen
- Readonly-Felder für State-Parameter und Zeitstempel
- Visualisierung des State-Status (aktiv/abgelaufen)
- Admin-Action zum Bereinigen abgelaufener States
- Sicherheit: Keine manuelle Erstellung/Änderung von States möglich
...
class OAuthStateAdmin(admin.ModelAdmin):
    ...
    search_fields = ["state", "user_identifier"]
    ...
    def status_indicator(self, obj):
        ...
        if obj.is_expired():
            return format_html('<span style="color: #dc3545;">🔴 Abgelaufen</span>')
        else:
    ...
    def cleanup_expired_states(self, request, queryset):
        ...
        if count > 0:
            self.message_user(
                request, f"✅ {count} abgelaufene OAuth States wurden gelöscht."
            )
        else:
            self.message_user(request, "✅ Keine abgelaufenen OAuth States gefunden.")
