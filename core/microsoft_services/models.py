"""
Microsoft Services Models - DSP (Digital Solutions Platform)

Dieses Modul enthält persistente Datenmodelle für Microsoft OAuth States und User-Mappings.
Ermöglicht sichere Speicherung und Verwaltung von OAuth-States für CSRF-Schutz und
stateless Deployments.

Features:
- Persistente Speicherung von OAuth States mit TTL und One-Time-Use
- Cleanup-Mechanismen für abgelaufene States
- Logging und Monitoring für Security Audits
- Erweiterbar für zukünftige Microsoft-Integrationsmodelle
...
class OAuthState(models.Model):
    ...
    def create_state(cls, state: str, user_identifier: str = None, timeout: int = 600):
        ...
        oauth_state = cls.objects.create(
            state=state, user_identifier=user_identifier, expires_at=expires_at
        )

        logger.info(
            f"OAuth state created in database: {state[:8]}... (expires: {expires_at})"
        )
        return oauth_state
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if user_identifier and oauth_state.user_identifier != user_identifier:
                    logger.warning(
                        f"OAuth state user identifier mismatch: {state[:8]}..."
                    )
                    return False
