...
class RoleAuthenticator:
    ...
    def __init__(
        self, custom_role_mappings: Optional[Dict[str, RoleConfiguration]] = None
    ):
        """
        ...
        logger.info(
            f"RoleAuthenticator initialized with {len(self._role_mappings)} role mappings"
        )

    def _initialize_role_mappings(
        self, custom_mappings: Optional[Dict[str, RoleConfiguration]] = None
        ...
        try:
            ...
            if not self._validate_email_format(user_email):
                return RoleAssignmentResult(
                    success=False, error_message=f"Invalid email format: {user_email}"
                )
            ...
            role_config, assignment_reason = self._determine_role_from_groups(
                user_groups
            )
            ...
            logger.error(
                f"Role assignment failed for {user_email}: {str(e)}", exc_info=True
            )
            return RoleAssignmentResult(
                success=False,
    ...
    def _get_user_groups_cached(
        self, user_id: str, user_email: str
    ) -> Optional[List[str]]:
        """
    ...
    def _get_user_groups(self, user_id: str) -> Optional[List[str]]:
        ...
        try:
            ...
            groups_query = (
                f"users/{user_id}/memberOf?$select=displayName,id,securityEnabled"
            )
    ...
    def _determine_role_from_groups(
        self, user_groups: List[str]
    ) -> Tuple[RoleConfiguration, str]:
        """
        ...
        if assigned_role:
            ...
            return self._role_mappings[
                "default"
            ], "no matching groups, using default role"
    ...
    def validate_role_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_response = self.graph_mixin.call_graph_api(
                "organization?$top=1&$select=displayName"
            )
            validation_result["configuration"]["graph_api_accessible"] = True
            validation_result["configuration"]["organization_accessible"] = bool(
                test_response.get("value")
            )
        ...
        try:
            ...
            validation_result["errors"].append(
                f"Role configuration validation failed: {str(e)}"
            )
