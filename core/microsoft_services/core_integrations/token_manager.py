...
class AzureTokenManager:
    ...
    def get_access_token(self, force_refresh: bool = False) -> str:
        ...
        with self._lock:
            ...
            if not access_token:
                raise AzureAuthException(
                    "No access_token in Azure AD response", auth_step="token_extraction"
                )

            # Calculate cache timeout with buffer
            expires_in = token_data.get("expires_in", 3600)  # Default 1 hour
            cache_timeout = max(
                expires_in - self.TOKEN_BUFFER_SECONDS, 60
            )  # Min 1 minute
    ...
    def _request_token_from_azure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AzureAuthException(
                f"Failed to connect to Azure AD: {str(e)}", auth_step="connection_error"
            )
        except requests.exceptions.RequestException as e:
            raise AzureAuthException(
                f"Azure AD token request failed: {str(e)}", auth_step="request_error"
            )
        ...
        if not token:
            try:
                ...
                return {
                    ...
                    "test_duration_ms": (datetime.now() - test_start).total_seconds()
                    * 1000,
                }
