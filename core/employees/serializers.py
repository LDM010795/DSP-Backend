...
class DepartmentSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        ...
        if self.instance:
            ...
            if (
                Department.objects.exclude(id=self.instance.id)
                .filter(name=value)
                .exists()
            ):
...
class PositionSerializer(serializers.ModelSerializer):
    ...
    def validate_title(self, value):
        ...
        if self.instance:
            # Bei Update: ausschließen des aktuellen Eintrags
            if (
                Position.objects.exclude(id=self.instance.id)
                .filter(title=value)
                .exists()
            ):
                raise serializers.ValidationError(
                    "Eine Position mit diesem Titel existiert bereits."
                )
        else:
...
class EmployeeSerializer(serializers.ModelSerializer):
    ...
    def validate_department(self, value):
        ...
        if not value.is_active:
            raise serializers.ValidationError(
                "Die ausgewählte Abteilung ist nicht aktiv."
            )
        return value
    ...
    def validate_position(self, value):
        ...
        if not value.is_active:
            raise serializers.ValidationError(
                "Die ausgewählte Position ist nicht aktiv."
            )
        return value
