...
class WordProcessingService:
    ...
    def process_word_document(
        self, file_content: bytes, file_name: str, cloud_url: str
        ...
        try:
            ...
            print(
                "[DEBUG] WordProcessingService: Übergabe an WordExtraction (mit Tabellenmarkern)"
            )
    ...
    def _extract_text_from_docx(self, doc: Document) -> str:
        ...
        for child in body.iterchildren():
            if isinstance(child, CT_P):
                ...
                if headers:
                    print(
                        f"[DEBUG] _extract_text_from_docx: Tabelle gefunden mit {len(headers)} Spalten und {len(data_rows)} Zeilen"
                    )
                    ...
                    print(
                        "[DEBUG] _extract_text_from_docx: Tabelle übersprungen (keine Header erkannt)"
                    )
                    continue
            else:
    ...
    def process_multiple_documents(
        ...
        logger.info(
            f"{len(processed_articles)} von {len(documents)} Dokumenten erfolgreich verarbeitet"
        )
