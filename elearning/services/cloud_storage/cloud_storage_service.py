...
class CloudStorageService:
    ...
    def _initialize_client(self) -> None:
        ...
        try:
            ...
            self.client = boto3.client(
                ...
                endpoint_url=os.getenv('WASABI_ENDPOINT_URL', 'https://s3.eu-central-2.wasabisys.com')
            )
            logger.info("Wasabi Cloud Storage Client initialisiert")
        except NoCredentialsError:
            logger.error("Wasabi Credentials nicht gefunden")
        except Exception as e:
            logger.error(f"Fehler beim Initialisieren des Wasabi Cloud Storage Clients: {e}")
    ...
    def get_module_content(self, module_name: str) -> Optional[ModuleContent]:
        ...
        try:
            ...
            logger.error(f"Fehler beim Abrufen des Modul-Inhalts für {module_name}: {e}")
            return None
    ...
    def _get_images_for_module(self, module_name: str) -> List[CloudFile]:
        ...
        try:
            ...
            logger.info(f"Gefundene Bilder für {module_name}: {[img.name for img in images]}")
            return images
            ...
            if 'Contents' in response:
                for obj in response['Contents']:
                    ...
                    cloud_file = CloudFile(
                        ...
                        last_modified=obj['LastModified'],
                        url=cloud_url,
                        content_type=obj.get('ContentType', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')
                    )
                    articles.append(cloud_file)
            
            logger.info(f"Gefundene Artikel für {module_name}: {[art.name for art in articles]}")
            return articles
    ...
    def download_file_content(self, object_key: str) -> Optional[bytes]:
        ...
        if not self.client:
            print(f"❌ [DEBUG] Cloud Storage Client nicht verfügbar")
            logger.error("Cloud Storage Client nicht verfügbar")
            return None
        
        try:
            print(f"🔍 [DEBUG] Cloud Storage: Versuche GetObject für Bucket: {self.bucket_name}, Key: {object_key}")
            ...
            print(f"✅ [DEBUG] Cloud Storage: Datei erfolgreich heruntergeladen ({len(content)} Bytes)")
            logger.info(f"Datei {object_key} erfolgreich heruntergeladen ({len(content)} Bytes)")
            return content
            ...
            logger.error(f"Unerwarteter Fehler beim Herunterladen von {object_key}: {e}")
            return None
