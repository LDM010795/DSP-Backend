...
class DatabaseService:
        ...
        try:
            ...
            try:
                ...
                default_category, created = ModuleCategory.objects.get_or_create(
                    name="Standard", defaults={"name": "Standard"}
                )

                module = Module.objects.create(
                    title=module_name, category=default_category, is_public=True
                )
                ...
                self.logger.error(
                    f"Fehler beim Erstellen des Moduls {module_name}: {e}"
                )
                return None

    def save_article_images(
        self, module: Module, images: List[Dict[str, Any]]
    ) -> List[ArticleImage]:
        """
            ...
            for image_data in images:
                try:
                    ...
                    existing_image = ArticleImage.objects.filter(
                        module=module, image_name=image_data["name"]
                    ).first()
                    ...
                    if existing_image:
                        ...
                        self.logger.info(
                            f"Neues Bild gespeichert: {image_data['name']}"
                        )

                except Exception as e:
                    self.logger.error(
                        f"Fehler beim Speichern des Bildes {image_data['name']}: {e}"
                    )
                    continue

        self.logger.info(
            f"{len(saved_images)} Bilder für Modul {module.title} gespeichert"
        )
        return saved_images
            ...
            for article_data in articles:
                try:
                    ...
                    if existing_article:
                        ...
                        self.logger.info(
                            f"Artikel aktualisiert: {article_data['title']}"
                        )
                    else:
                        ...
                        self.logger.info(
                            f"Neuer Artikel gespeichert: {article_data['title']}"
                        )

                except Exception as e:
                    self.logger.error(
                        f"Fehler beim Speichern des Artikels {article_data['title']}: {e}"
                    )
                    continue
    ...
    def process_module_content(
        ...
        try:
            ...
            self.logger.info(
                f"Modul {module_name} erfolgreich verarbeitet: "
    ...
    def get_module_statistics(self, module_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"module_name": module_name, "error": "Modul nicht gefunden"}
        except Exception as e:
    ...
    def validate_module_data(
        self, module_name: str, images: List[Dict], articles: List[Dict]
    ) -> Dict[str, Any]:
        """
