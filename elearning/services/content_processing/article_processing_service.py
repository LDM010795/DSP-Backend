...
class ArticleProcessingService:
    ...
    def process_article_from_cloud_url(self, module_id: int, cloud_url: str, chapter_id: Optional[int] = None) -> ProcessedArticleResult:
        ...
        try:
            ...
            if not parsed_url:
                print(f"❌ [DEBUG] Cloud-URL konnte nicht geparst werden")
            ...
            file_content = self.cloud_service.download_file_content(parsed_url['object_key'])
            if not file_content:
                print(f"❌ [DEBUG] Word-Dokument konnte nicht heruntergeladen werden")
                result.errors.append("Konnte Word-Dokument nicht herunterladen")
                return result
            
            print(f"✅ [DEBUG] Word-Dokument erfolgreich heruntergeladen ({len(file_content)} Bytes)")
            
            # 3. Word-Dokument verarbeiten
            print(f"🔍 [DEBUG] Verarbeite Word-Dokument mit WordExtraction")
            processed_article = self.word_service.process_word_document(
                file_content, 
                parsed_url['file_name'], 
                cloud_url
            )
            
            if not processed_article:
                print(f"❌ [DEBUG] Word-Dokument-Verarbeitung fehlgeschlagen")
            ...
            print(f"🔍 [DEBUG] Extrahiere Bilder aus JSON")
            images_from_json = self._extract_images_from_json(processed_article.json_content)
            ...
            print(f"🔍 [DEBUG] Speichere Artikel in Datenbank")
            article_data = {
            ...
            if images_from_json:
                print(f"🔍 [DEBUG] Verarbeite Bilder für Artikel")
                images_saved = self._process_images_for_article(
                    module, 
                    parsed_url['module_name'], 
                    images_from_json
                )
            ...
            result.success = True
            print(f"🎉 [DEBUG] Artikel '{processed_article.title}' erfolgreich verarbeitet")
            self.logger.info(f"Artikel '{processed_article.title}' erfolgreich verarbeitet")
    ...
    def _process_images_for_article(self, module, module_name: str, image_names: List[str]) -> int:
        """
        ...
        try:
            ...
            for image in module_content.images:
                image_name_without_ext = image.name.split('.')[0] if '.' in image.name else image.name
            ...
            if matching_images:
                saved_images = self.db_service.save_article_images(module, matching_images)
    ...
    def validate_cloud_url(self, cloud_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not parsed_info['file_name'].lower().endswith('.docx'):
                validation_result['errors'].append("URL muss auf eine .docx Datei zeigen")
                return validation_result
