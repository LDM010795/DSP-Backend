        ...
        try:
            ...
            if db_result["success"]:
                ...
                result.errors.append(
                    db_result.get("error", "Unbekannter Datenbankfehler")
                )

        except Exception as e:
            error_msg = (
                f"Unerwarteter Fehler bei der Verarbeitung von {module_name}: {str(e)}"
            )
    ...
    def _process_images(self, images: List[CloudFile]) -> List[Dict[str, Any]]:
        ...
        for image in images:
            try:
                ...
                self.logger.error(
                    f"Fehler beim Verarbeiten des Bildes {image.name}: {e}"
                )
                continue
    ...
    def _process_articles(self, articles: List[CloudFile]) -> List[Dict[str, Any]]:
        ...
        for article in articles:
            try:
                ...
                if not file_content:
                    self.logger.error(
                        f"Konnte Word-Dokument {article.name} nicht herunterladen"
                    )
                    continue
                ...
                if processed_article:
                    ...
                    self.logger.error(
                        f"Fehler beim Verarbeiten des Word-Dokuments {article.name}"
                    )

            except Exception as e:
                self.logger.error(
                    f"Fehler beim Verarbeiten des Artikels {article.name}: {e}"
                )
                continue
    ...
    def process_multiple_modules(
        ...
        for module_name in module_names:
            ...
            if result.success:
                ...
                self.logger.error(
                    f"Modul {module_name} Verarbeitung fehlgeschlagen: {result.errors}"
                )
