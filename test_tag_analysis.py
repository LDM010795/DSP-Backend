...
def load_real_word_document(file_path: str) -> str:
    ...
    try:
        ...
        for i, para in enumerate(paragraphs[:10]):
            ...
            if text:
                print(f"   Absatz {i+1}: {text[:50]}{'...' if len(text) > 50 else ''}")
...
def repair_existing_json_files(output_dir: str):
    ...
    json_files = [f for f in os.listdir(output_dir) if f.endswith('_extracted_content.json')]
            ...
            for block in data.get('content', []):
                if block.get('type') == 'table_of_contents':
                    ...
                    if 'items' in block and len(block['items']) == 1:
                        ...
                        if len(items) > 1:
                            block['items'] = items
                            print(f"  ✅ Inhaltsverzeichnis repariert: {len(block['items'])} Items")
                        elif text == '-':
                    ...
                    if 'items' in block and len(block['items']) == 1:
                        ...
                        if len(items) > 1:
                            block['items'] = items
                            print(f"  ✅ Lernziele repariert: {len(block['items'])} Items")
                    ...
                    if 'items' in block and len(block['items']) == 1:
                        ...
                        if len(items) > 1:
                            block['items'] = items
                            print(f"  ✅ Auflistung repariert: {len(block['items'])} Items")
...
def print_tag_analysis(analysis: dict, filename: str):
    ...
    print(f"   • Verschiedene Tag-Arten verarbeitet: {summary['different_processed_tags']}")
    print(f"   • Gesamte verarbeitete Vorkommen: {summary['total_processed_occurrences']}")
    print(f"   • Verschiedene unbekannte Tag-Arten: {summary['different_unknown_tags']}")
    ...
    all_tags = ['Titel$', 'Titel2$', 'Titel3$', 'Text$', 'Hinweis$', 'Exkurs$', 
                'Quellen$', 'Lernziele$', 'Inhaltsverzeichnis$', 'Auflistung$', 
                'Wichtig$', 'Tipp$', 'Bild$', 'Code$']
...
def process_single_word_file(file_path: str, output_dir: str) -> bool:
    ...
    print(f"📊 Verschiedene Tag-Arten: {tag_analysis['summary']['different_found_tags']}")
    print(f"📊 Gesamte Tag-Vorkommen: {tag_analysis['summary']['total_found_occurrences']}")
    print(f"📊 Verarbeitete Tag-Arten: {tag_analysis['summary']['different_processed_tags']}")
    print(f"📊 Verarbeitete Vorkommen: {tag_analysis['summary']['total_processed_occurrences']}")
...
def main():
    ...
    print(f"📁 Ausgabeverzeichnis: {output_dir}")
    print(f"📄 Erstellte JSON-Dateien:")
